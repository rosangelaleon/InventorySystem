...
public class Form_Colores extends javax.swing.JInternalFrame implements InterfaceUsuario {
    ...
    private void setMapData() {
        mapData.clear();
        String id = Idcolor.getText().trim();
        if (!id.isEmpty() && !id.equals("0")) {
            mapData.put("id", id);
        }

        String nombre = color.getText().trim();
        if (!nombre.isEmpty()) {
            mapData.put("color", nombre);
        }
...
private void fillView(Map<String, String> data) {
    Idcolor.setText(data.getOrDefault("id", ""));
    color.setText(data.getOrDefault("color", ""));
    textdescripcion.setText(data.getOrDefault("descripcion", ""));
}


 private void initializeTextFields() {
        applyNumericFilter(Idcolor);
        applyAlphaFilter(color);
...
Idcolor.addFocusListener(new DefaultFocusListener(Idcolor, true)); // Aún rellenará con "0"
color.addFocusListener(new DefaultFocusListener(color, false)); // Aún rellenará con "0"
...
private void addEnterKeyListenerToIdField() {
    Idcolor.addKeyListener(new KeyAdapter() {
    ...
    private int obtenerIdActual() {
        try {
            return Integer.parseInt(Idcolor.getText().trim());
...
private int procesarRegistroNavegacion(Map<String, String> registro) {
    if (registro == null || registro.isEmpty()) {
        ...
        String nombre = registro.get("color");
...
private void actualizarUIConRegistro(int id, String nombre,String descripcion) {
    this.idColor = id;
    Idcolor.setText(String.valueOf(id));
...
private void buscarYRellenarDatos() {
    ...
    if (!id.isEmpty()) {
        ...
        try {
            ...
            List<Map<String, String>> registros = tc.buscarPorIdGenerico("COLORES", "id", idT);
            idColor=idT;
            if (!registros.isEmpty()) {
                ...
                JOptionPane.showMessageDialog(this, "No se encontró un color con el ID especificado.", "Información", JOptionPane.INFORMATION_MESSAGE);
                 resetData(); 
            }
        ...
        layout.setHorizontalGroup(
            ...
            .addGroup(layout.createSequentialGroup()
                ...
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addComponent(color)
                    .addGroup(layout.createSequentialGroup()
                        .addComponent(Idcolor, javax.swing.GroupLayout.PREFERRED_SIZE, 72, javax.swing.GroupLayout.PREFERRED_SIZE)
...
public int imGuardar(String crud) {
    ...
    if (id.isEmpty() || Integer.parseInt(id) <= 0 || nombre.isEmpty()) {
        JOptionPane.showMessageDialog(null, "Los campos ID y color son obligatorios y deben ser válidos.",
    ...
    if (rowsAffected > 0) {
        JOptionPane.showMessageDialog(null, isUpdate ? "Color actualizado correctamente." : "Color creado correctamente.",
...
private boolean Existe(String id) {
    ...
    System.out.println("Buscando color con ID: " + id);
    ...
    if (Data != null && !Data.isEmpty()) {
        ...
        System.out.println("No se encontró ningun color con el ID: " + id);
        return false;
    }
}
    ...
    public int imBorrar(String crud) {
     if (Idcolor.getText().trim().isEmpty() || color.getText().trim().isEmpty()) {
        ...
        int confirmacion = JOptionPane.showConfirmDialog(this, "¿Está seguro de que desea eliminar este color?", "Confirmar", JOptionPane.YES_NO_OPTION);
        ...
        if (resultado > 0) {
            JOptionPane.showMessageDialog(this, "Color eliminada con éxito.", "Éxito", JOptionPane.INFORMATION_MESSAGE);
        } else {
            JOptionPane.showMessageDialog(this, "No se pudo eliminar el color", "Error", JOptionPane.ERROR_MESSAGE);
        }
...
public int imBuscar() { 
    ...
    List<String> columnas= Arrays.asList("id", "color");
    Form_Buscar buscador = new Form_Buscar(parentFrame, true, tc, "COLORES", columnas);
...
public int imSiguiente() {
    ...
    Map<String, String> registro = tc.navegationReg(String.valueOf(idColor ), "NEXT");
    return procesarRegistroNavegacion(registro);
}
...
public int imAnterior() {
    ...
    Map<String, String> registro = tc.navegationReg(String.valueOf(idColor ), "PRIOR");
    return procesarRegistroNavegacion(registro);
}
...
public void onItemSeleccionado(Map<String, String> datosSeleccionados) {
    ...
    if (idStr != null && !idStr.isEmpty()) {
        try {
            ...
            List<Map<String, String>> registros = tc.buscarPorIdGenerico("COLORES", "id",  id );
            ...
            if (!registros.isEmpty()) {
                ...
                SwingUtilities.invokeLater(() -> {
                    Idcolor.setText(idStr);
                    color.setText(nombre);
                    textdescripcion.setText(descripcion); // Actualizar campo de descripción
                   idColor = id; // Guardar el ID de la  actual
                });
            } else {
                JOptionPane.showMessageDialog(null, "No se encontró un color con el Código especificado.", "Información", JOptionPane.INFORMATION_MESSAGE);
            }
        } catch (NumberFormatException ex) {
