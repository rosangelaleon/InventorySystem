...
public class Form_Diseños extends javax.swing.JInternalFrame implements InterfaceUsuario {
    private DBTableController tc;
    private Map<String, String> mapData;
    private int idDisenar;
    ...
    private void setMapData() {
        mapData.clear();
        String id = IdDiseno.getText().trim();
        if (!id.isEmpty() && !id.equals("0")) {
            mapData.put("id", id);
        }

        String nombrediseno = diseno.getText().trim();
        if (!nombrediseno.isEmpty()) {
            mapData.put("diseno", nombrediseno);
        }
...
private void fillView(Map<String, String> data) {
    IdDiseno.setText(data.getOrDefault("id", ""));
    diseno.setText(data.getOrDefault("diseno", ""));
    textdescripcion.setText(data.getOrDefault("descripcion", ""));
}


 private void initializeTextFields() {
        applyNumericFilter(IdDiseno);
        applyAlphaFilter(diseno);
...
IdDiseno.addFocusListener(new DefaultFocusListener(IdDiseno, true)); // Aún rellenará con "0"
diseno.addFocusListener(new DefaultFocusListener(diseno, false)); // Aún rellenará con "0"
...
private void addEnterKeyListenerToIdField() {
    IdDiseno.addKeyListener(new KeyAdapter() {
    ...
    private int obtenerIdActual() {
        try {
            return Integer.parseInt(IdDiseno.getText().trim());
...
private int procesarRegistroNavegacion(Map<String, String> registro) {
    if (registro == null || registro.isEmpty()) {
        ...
        int IdDiseno = Integer.parseInt(registro.get("id"));
        String nombrediseno = registro.get("diseno");
        String descripciondiseno = registro.get("descripcion");
        actualizarUIConRegistro(IdDiseno, nombrediseno,descripciondiseno);
        return 1;
    }
}

private void actualizarUIConRegistro(int id, String nombrediseno,String descripciondiseno) {
    this.idDisenar = id;
    IdDiseno.setText(String.valueOf(id));
    diseno.setText(nombrediseno);
        textdescripcion.setText(descripciondiseno != null ? descripciondiseno : "");
}
private void buscarYRellenarDatos() {
    ...
    if (!id.isEmpty()) {
         resetData();
        try {
            int idDiseno = Integer.parseInt(id);
            List<Map<String, String>> registros = tc.buscarPorIdGenerico("DISENOS", "id", idDiseno);
            idDisenar=idDiseno;
            if (!registros.isEmpty()) {
                ...
                JOptionPane.showMessageDialog(this, "No se encontró un diseño con el ID especificado.", "Información", JOptionPane.INFORMATION_MESSAGE);
                 resetData(); 
            }
    ...
    private void initComponents() {
        ...
        IdDiseno = new javax.swing.JTextField();
        ...
        setClosable(true);
        setIconifiable(true);
        setTitle("Registrar Diseños");

        jPanel1.setLayout(new org.netbeans.lib.awtextra.AbsoluteLayout());

        lbl_id.setText("Id");

        lbl_diseno.setText("Diseño");
        ...
        layout.setHorizontalGroup(
            ...
            .addGroup(layout.createSequentialGroup()
                ...
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addComponent(lbl_diseno, javax.swing.GroupLayout.Alignment.TRAILING)
                    .addComponent(lbl_id, javax.swing.GroupLayout.Alignment.TRAILING))
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addComponent(diseno)
                    .addGroup(layout.createSequentialGroup()
                        .addComponent(IdDiseno, javax.swing.GroupLayout.PREFERRED_SIZE, 72, javax.swing.GroupLayout.PREFERRED_SIZE)
        ...
        layout.setVerticalGroup(
            ...
            .addGroup(layout.createSequentialGroup()
                ...
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                    .addComponent(IdDiseno, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
                    .addComponent(lbl_id)
                    .addComponent(lblultimo)
    ...
    private javax.swing.JTextField IdDiseno;
...
public int imGuardar(String crud) {
    ...
    if (id.isEmpty() || Integer.parseInt(id) <= 0 || nombrediseno.isEmpty()) {
        JOptionPane.showMessageDialog(null, "Los campos ID y diseño son obligatorios y deben ser válidos.",
    ...
    setMapData();
    boolean isUpdate = disenoExiste(id);
    int rowsAffected = 0;

    if (isUpdate) {
        Map<String, String> existingData = tc.searchById(mapData);  // Asumimos que este método devuelve los datos actuales del diseño
    ...
    if (rowsAffected > 0) {
        JOptionPane.showMessageDialog(null, isUpdate ? "Diseño actualizado correctamente." : "Diseño creado correctamente.",
    ...
    fillView(mapData);
    actualizarUltimoId();
    IdDiseno.requestFocusInWindow();

    return rowsAffected;
}

private boolean disenoExiste(String id) {
    ...
    System.out.println("Buscando diseño con ID: " + id);

    // Usa el método searchById para buscar 
    Map<String, String> disenoData = tc.searchById(params);  // Asumiendo que tc es tu controlador de base de datos para la tabla

    if (disenoData != null && !disenoData.isEmpty()) {
        return true;
    } else {
        System.out.println("No se encontró ningun diseño con el ID: " + id);
        return false;
    }
}
    ...
    public int imBorrar(String crud) {
     if (IdDiseno.getText().trim().isEmpty() || diseno.getText().trim().isEmpty()) {
        ...
        int idDisenoActual = obtenerIdActual();
        if (idDisenoActual <= 0) {
            JOptionPane.showMessageDialog(this, "ID inválido o no seleccionado.", "Error", JOptionPane.ERROR_MESSAGE);
            return -1;
        }

        int confirmacion = JOptionPane.showConfirmDialog(this, "¿Está seguro de que desea eliminar esta diseño?", "Confirmar", JOptionPane.YES_NO_OPTION);
        ...
        registro.put("id", String.valueOf(idDisenoActual));
        registrosParaBorrar.add(registro);

        int resultado = tc.deleteReg(registrosParaBorrar);
        if (resultado > 0) {
            JOptionPane.showMessageDialog(this, "Diseño eliminada con éxito.", "Éxito", JOptionPane.INFORMATION_MESSAGE);
        } else {
            JOptionPane.showMessageDialog(this, "No se pudo eliminar el diseño.", "Error", JOptionPane.ERROR_MESSAGE);
        }
...
public int imBuscar() {
 ...
 resetData(); 
    ...
    List<String> columnasParadiseno = Arrays.asList("id", "diseno");
    Form_Buscar buscadordisenos = new Form_Buscar(parentFrame, true, tc, "DISENOS", columnasParadiseno);

    buscadordisenos.setOnItemSeleccionadoListener(this);
    // Muestra el formulario de búsqueda
    buscadordisenos.setVisible(true);
...
public int imSiguiente() {
    ...
    Map<String, String> registro = tc.navegationReg(String.valueOf(idDisenar), "NEXT");
    return procesarRegistroNavegacion(registro);
}
...
public int imAnterior() {
    ...
    Map<String, String> registro = tc.navegationReg(String.valueOf(idDisenar), "PRIOR");
    return procesarRegistroNavegacion(registro);
}
...
public void onItemSeleccionado(Map<String, String> datosSeleccionados) {
    ...
    String nombrediseno = datosSeleccionados.get("Descripcion"); // 'Descripcion' se usa como clave para el nombre 

    if (idStr != null && !idStr.isEmpty()) {
        try {
            int idDiseno = Integer.parseInt(idStr); // Convertir el ID a entero
            ...
            if (!registros.isEmpty()) {
                ...
                final String descripciondiseno = registro.get("descripcion"); // Recuperar la descripción si está disponible

                // Actualizar la UI en el hilo de Swing
                SwingUtilities.invokeLater(() -> {
                    IdDiseno.setText(idStr);
                    diseno.setText(nombrediseno);
                    textdescripcion.setText(descripciondiseno); // Actualizar campo de descripción
                    idDisenar = idDiseno; // Guardar el ID de la  actual
                });
            } else {
                JOptionPane.showMessageDialog(null, "No se encontró un diseño con el Código especificado.", "Información", JOptionPane.INFORMATION_MESSAGE);
            }
        } catch (NumberFormatException ex) {
