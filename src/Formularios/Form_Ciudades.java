...
public class Form_Ciudades extends javax.swing.JInternalFrame implements InterfaceUsuario {
    ...
    private void setMapData() {
        mapData.clear();
        String id = Idciudad.getText().trim();
        if (!id.isEmpty() && !id.equals("0")) {
            mapData.put("id", id);
        }

        String nombre = ciudad.getText().trim();
        if (!nombre.isEmpty()) {
            mapData.put("ciudad", nombre);
        }
...
private void fillView(Map<String, String> data) {
    Idciudad.setText(data.getOrDefault("id", ""));
    ciudad.setText(data.getOrDefault("ciudad", ""));
    textdescripcion.setText(data.getOrDefault("descripcion", ""));
}


 private void initializeTextFields() {
        applyNumericFilter(Idciudad);
        applyAlphaFilter(ciudad);
...
Idciudad.addFocusListener(new DefaultFocusListener(Idciudad, true)); // Aún rellenará con "0"
ciudad.addFocusListener(new DefaultFocusListener(ciudad, true)); // Aún rellenará con "0"
...
private void addEnterKeyListenerToIdField() {
    ...
    private int obtenerIdActual() {
        try {
            return Integer.parseInt(Idciudad.getText().trim());
...
private int procesarRegistroNavegacion(Map<String, String> registro) {
    if (registro == null || registro.isEmpty()) {
        ...
        String nombre = registro.get("ciudad");
...
private void actualizarUIConRegistro(int id, String nombre,String descripcion) {
    this.idCiudad = id;
    Idciudad.setText(String.valueOf(id));
...
private void buscarYRellenarDatos() {
    ...
    if (!id.isEmpty()) {
        ...
        try {
            ...
            List<Map<String, String>> registros = tc.buscarPorIdGenerico("CIUDADES", "id", idT);
            idCiudad=idT;
            if (!registros.isEmpty()) {
                ...
                JOptionPane.showMessageDialog(this, "No se encontró una ciudad con el ID especificado.", "Información", JOptionPane.INFORMATION_MESSAGE);
                 resetData(); 
            }
        ...
        layout.setHorizontalGroup(
            ...
            .addGroup(layout.createSequentialGroup()
                ...
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addComponent(lbl_ciudad, javax.swing.GroupLayout.Alignment.TRAILING)
                    .addComponent(lbl_id, javax.swing.GroupLayout.Alignment.TRAILING))
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addComponent(ciudad)
                    .addGroup(layout.createSequentialGroup()
                        .addComponent(Idciudad, javax.swing.GroupLayout.PREFERRED_SIZE, 72, javax.swing.GroupLayout.PREFERRED_SIZE)
...
public int imGuardar(String crud) {
    ...
    if (id.isEmpty() || Integer.parseInt(id) <= 0 || nombre.isEmpty()) {
        JOptionPane.showMessageDialog(null, "Los campos ID y ciudad son obligatorios y deben ser válidos.",
    ...
    if (rowsAffected > 0) {
        JOptionPane.showMessageDialog(null, isUpdate ? "Ciudad actualizado correctamente." : "Ciudad creado correctamente.",
...
private boolean Existe(String id) {
    ...
    System.out.println("Buscando ciudad con ID: " + id);
    ...
    if (Data != null && !Data.isEmpty()) {
        ...
        System.out.println("No se encontró ningun ciudad con el ID: " + id);
        return false;
    }
}
    ...
    public int imBorrar(String crud) {
     if (Idciudad.getText().trim().isEmpty() || ciudad.getText().trim().isEmpty()) {
        ...
        int confirmacion = JOptionPane.showConfirmDialog(this, "¿Está seguro de que desea eliminar este ciudad?", "Confirmar", JOptionPane.YES_NO_OPTION);
        ...
        if (resultado > 0) {
            JOptionPane.showMessageDialog(this, "Ciudad eliminada con éxito.", "Éxito", JOptionPane.INFORMATION_MESSAGE);
        } else {
            JOptionPane.showMessageDialog(this, "No se pudo eliminar el ciudad", "Error", JOptionPane.ERROR_MESSAGE);
        }
...
public int imBuscar() { 
  ...
  resetData(); 
    ...
    List<String> columnas= Arrays.asList("id", "ciudad");
    Form_Buscar buscador = new Form_Buscar(parentFrame, true, tc, "CIUDADES", columnas);
...
public int imSiguiente() {
    ...
    Map<String, String> registro = tc.navegationReg(String.valueOf(idCiudad ), "NEXT");
    return procesarRegistroNavegacion(registro);
}
...
public int imAnterior() {
    ...
    Map<String, String> registro = tc.navegationReg(String.valueOf(idCiudad ), "PRIOR");
    return procesarRegistroNavegacion(registro);
}
...
public void onItemSeleccionado(Map<String, String> datosSeleccionados) {
    ...
    if (idStr != null && !idStr.isEmpty()) {
        try {
            ...
            List<Map<String, String>> registros = tc.buscarPorIdGenerico("CIUDADES", "id",  id );
            ...
            if (!registros.isEmpty()) {
                ...
                SwingUtilities.invokeLater(() -> {
                    Idciudad.setText(idStr);
                    ciudad.setText(nombre);
                    textdescripcion.setText(descripcion); // Actualizar campo de descripción
                   idCiudad = id; // Guardar el ID de la  actual
                });
            } else {
                JOptionPane.showMessageDialog(null, "No se encontró un ciuddad con el Código especificado.", "Información", JOptionPane.INFORMATION_MESSAGE);
            }
        } catch (NumberFormatException ex) {
