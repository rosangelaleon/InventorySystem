...
public class Form_Cajas extends javax.swing.JInternalFrame implements InterfaceUsuario{
    ...
    public Form_Cajas() {
        initComponents();
        caja_id.setText("0"); 
        nombre_caja.setText(""); // Limpiar campo Nombre
    ...
    applyNumericFilter(caja_id);
    applyAlphaFilter(nombre_caja);
    addFocusListeners();
    ...
    private void addFocusListeners() {
        caja_id.addFocusListener(new DefaultFocusListener(caja_id,false));
        nombre_caja.addFocusListener(new DefaultFocusListener(nombre_caja,true));
    }
    
    private void addEnterKeyListenerToIdField() {
    caja_id.addKeyListener(new KeyAdapter() {
        public void keyPressed(KeyEvent e) {
            if (e.getKeyCode() == KeyEvent.VK_ENTER) {
                int id = obtenerIdCajaActual();
                if (id != -1) {
                    List<Map<String, String>> resultado = tc.buscarPorIdGenerico("CAJAS", "id", id);
                    if (!resultado.isEmpty()) {
                        Map<String, String> datos = resultado.get(0); // Asumimos que la búsqueda por ID devuelve un único resultado
                        fillView(datos);
                        idCaja=id;
                    } else {
...
private void fillView(Map<String, String> data) {
    caja_id.setText(data.getOrDefault("id", ""));
    nombre_caja.setText(data.getOrDefault("nombre", ""));
    ...
    private void actualizarUltimoId() {
        try {
            int ultimoId = tc.getMaxId(); // devuelve el ID máximo actual de la tabla 
            UltimoId.setText(String.valueOf(ultimoId));
        } catch (Exception e) {
            JOptionPane.showMessageDialog(this, "No se pudo cargar el último ID de la caja.", "Error", JOptionPane.ERROR_MESSAGE);
        }
    }
    
private void SetMapData() {
    mapData.put("id", caja_id.getText());
    mapData.put("nombre", nombre_caja.getText());
...
private void resetData() {
    mapData.put("id", "0");
    mapData.put("nombre", "");
    mapData.put("activo", "0"); // Asegúrate de resetear el campo activo también
...
private int obtenerIdCajaActual() {
    try {
        return Integer.parseInt(caja_id.getText().trim());
    } catch (NumberFormatException e) {
        JOptionPane.showMessageDialog(this, "ID inválido para la caja.", "Error", JOptionPane.ERROR_MESSAGE);
        return -1;
    }
}
...
private int procesarRegistroNavegacion(Map<String, String> registro) {
    if (registro == null || registro.isEmpty()) {
        ...
        int idCaja = Integer.parseInt(registro.get("id"));
        String nombreCaja = registro.get("nombre");
        actualizarUIConRegistro(idCaja, nombreCaja);
        return 1;
    }
}

private void actualizarUIConRegistro(int idCaja, String nombreCaja) {
    this.idCaja = idCaja;
    caja_id.setText(String.valueOf(idCaja));
    nombre_caja.setText(nombreCaja);
}
    ...
    private void initComponents() {
        ...
        jPanel1.add(caja_id, new org.netbeans.lib.awtextra.AbsoluteConstraints(62, 20, 93, -1));

        lbl_nombre.setText("Nombre");
        jPanel1.add(lbl_nombre, new org.netbeans.lib.awtextra.AbsoluteConstraints(6, 62, -1, -1));
        jPanel1.add(nombre_caja, new org.netbeans.lib.awtextra.AbsoluteConstraints(62, 62, 194, -1));
        ...
        jPanel1.add(Activo, new org.netbeans.lib.awtextra.AbsoluteConstraints(110, 100, -1, -1));
 ...
 private void roles_idActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_roles_idActionPerformed
   ...
   // Si se establece el texto aquí, podría sobrescribir el valor establecido programáticamente
    if (caja_id.getText().isEmpty()) {
        caja_id.setText("Ingrese ID de la caja"); // Solo establecer texto si está vacío
    }
    ...
    if (nombre_caja.getText().isEmpty()) {
        nombre_caja.setText("Ingrese nombre de la caja"); // Solo establecer texto si está vacío
    }
...
public int imGuardar(String crud) {
    ...
    String strId = caja_id.getText().trim();
    String nombreCaja = nombre_caja.getText().trim();
    String activo = Activo.isSelected() ? "1" : "0"; // Asegúrate de obtener el valor de `activo`
    
    if (strId.isEmpty() || strId.equals("0") || nombreCaja.isEmpty()) {
        JOptionPane.showMessageDialog(this, "Los campos ID y Nombre son obligatorios y deben ser válidos.", "Error", JOptionPane.ERROR_MESSAGE);
        resetData(); 
        fillView(mapData); 
        actualizarUltimoId(); 
        return -1; 
    ...
    if (tc.existAny(mapData) > 0) {
        ...
        msg = (rows > 0) ? "La caja ha sido actualizada con éxito." : "No se pudo actualizar la caja.";
    } else {
        rows = tc.createReg(mapData);
        msg = (rows > 0) ? "La caja ha sido creada con éxito." : "No se pudo crear la caja.";
    }
...
public int imBorrar(String crud) {
    if (caja_id.getText().trim().isEmpty() || nombre_caja.getText().trim().isEmpty()) {
    ...
    int idActual = obtenerIdCajaActual();
    if (idActual <= 0) {
        JOptionPane.showMessageDialog(this, "ID inválido o no seleccionado.", "Error", JOptionPane.ERROR_MESSAGE);
        return -1;
    }

    int confirmacion = mostrarDialogoConfirmacion("¿Está seguro de que desea eliminar esta caja?");
    ...
    if (resultado > 0) {
        ...
        JOptionPane.showMessageDialog(this, "No se pudo eliminar la caja,se utiliza en otra tabla.", "Error", JOptionPane.ERROR_MESSAGE);
    }
...
// Resetear los datos
    ...
    public int imFiltrar() {
        ...
        Form_Buscar buscador = new Form_Buscar(parentFrame, true, tc, "CAJAS", columnas);
...
public int imSiguiente() {
     // Si id es -1, significa que no hay registro actual seleccionado
    if (idCaja == -1) {
          JOptionPane.showMessageDialog(this, "Seleccione un registro primero.");
        return -1;
    }
    Map<String, String> registro = tc.navegationReg(String.valueOf(idCaja), "NEXT");
    return procesarRegistroNavegacion(registro);
}
...
if (idCaja <= 1) {
    ...
    if (idCaja <= 1) return -1;
    Map<String, String> registro = tc.navegationReg(String.valueOf(idCaja), "PRIOR");
    return procesarRegistroNavegacion(registro);
}
 ...
 PrinterJob job = PrinterJob.getPrinterJob();
    ...
    public int imBuscar() {
                ...
                if (id != -1) {
                    List<Map<String, String>> resultado = tc.buscarPorIdGenerico("CAJAS", "id", id);
                    if (!resultado.isEmpty()) {
                        Map<String, String> datosCaja = resultado.get(0); // Asumimos que la búsqueda por ID devuelve un único resultado
                        fillView(datosCaja);
                        idCaja=id;
                    } else {
                        JOptionPane.showMessageDialog(null, "No se encontró la caja con ID: " + id);
...
public void onItemSeleccionado(Map<String, String> datosSeleccionados) {
    ...
    final String nombre = normalizedData.get("Descripcion");
        final String activoStr = normalizedData.get("activo");
        final boolean activo = "1".equals(activoStr);
    System.out.println("ID: " + id + ", Nombre: " + nombre);

    SwingUtilities.invokeLater(() -> {
        caja_id.setText(id);
        nombre_caja.setText(nombre);
         idCaja = Integer.parseInt(id);
