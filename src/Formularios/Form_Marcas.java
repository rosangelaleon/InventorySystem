...
public class Form_Marcas extends javax.swing.JInternalFrame implements InterfaceUsuario {
    ...
    private void setMapData() {
        mapData.clear();
        String id = IdMarcas.getText().trim();
        if (!id.isEmpty() && !id.equals("0")) {
            mapData.put("id", id);
        }

        String nombreMarca = marca.getText().trim();
        if (!nombreMarca.isEmpty()) {
            mapData.put("marca", nombreMarca);
        }
...
private void fillView(Map<String, String> data) {
    ...
    IdMarcas.setText(data.getOrDefault("id", ""));
    marca.setText(data.getOrDefault("marca", ""));
    textdescripcion.setText(data.getOrDefault("descripcion", ""));
}


 private void initializeTextFields() {
        applyNumericFilter(IdMarcas);
        applyAlphaFilter(marca);
...
IdMarcas.addFocusListener(new DefaultFocusListener(IdMarcas, true)); // Aún rellenará con "0"
marca.addFocusListener(new DefaultFocusListener(marca, false)); // Aún rellenará con "0"
...
private void addEnterKeyListenerToIdField() {
    IdMarcas.addKeyListener(new KeyAdapter() {
    ...
    private int obtenerIdMarcasActual() {
        try {
            return Integer.parseInt(IdMarcas.getText().trim());
...
private int procesarRegistroNavegacion(Map<String, String> registro) {
    if (registro == null || registro.isEmpty()) {
        ...
        int idMarca = Integer.parseInt(registro.get("id"));
        String nombreMarca = registro.get("marca");
        String descripcionMarca = registro.get("descripcion");
        actualizarUIConRegistro(idMarca, nombreMarca,descripcionMarca);
        return 1;
    }
}

private void actualizarUIConRegistro(int idMarca, String nombreMarca, String descripcionMarca) {
    this.idMarcas = idMarca;
    IdMarcas.setText(String.valueOf(idMarca));
    marca.setText(nombreMarca);
    System.out.println("Descripción obtenida: " + descripcionMarca);  // Imprimir para depuración
    textdescripcion.setText(descripcionMarca != null ? descripcionMarca : "");
}
...
private void buscarYRellenarDatos() {
    ...
    if (!id.isEmpty()) {
        ...
        try {
            ...
            List<Map<String, String>> registros = tc.buscarPorIdGenerico("MARCAS", "id", idMarca);
            idMarcas=idMarca;
            if (!registros.isEmpty()) {
                ...
                JOptionPane.showMessageDialog(this, "No se encontró una marca con el ID especificado.", "Información", JOptionPane.INFORMATION_MESSAGE);
                 resetData(); 
            }
    ...
    private void initComponents() {
        ...
        IdMarcas = new javax.swing.JTextField();
        ...
        layout.setHorizontalGroup(
            ...
            .addGroup(layout.createSequentialGroup()
                ...
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addComponent(marca)
                    .addGroup(layout.createSequentialGroup()
                        .addComponent(IdMarcas, javax.swing.GroupLayout.PREFERRED_SIZE, 72, javax.swing.GroupLayout.PREFERRED_SIZE)
            ...
            .addGroup(layout.createSequentialGroup()
                ...
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                        .addComponent(IdMarcas, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
                        .addComponent(lbl_id)
                        .addComponent(lblultimo)))
                .addGap(18, 18, 18)
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
    ...
    private javax.swing.JTextField IdMarcas;
...
public int imGuardar(String crud) {
    ...
    if (id.isEmpty() || Integer.parseInt(id) <= 0 || nombreMarca.isEmpty()) {
        JOptionPane.showMessageDialog(null, "Los campos ID y Marca son obligatorios y deben ser válidos.", 
    ...
    setMapData();
    boolean isUpdate = marcaExiste(id);
    int rowsAffected = 0;
    ...
    if (rowsAffected > 0) {
        JOptionPane.showMessageDialog(null, isUpdate ? "Marca actualizada correctamente." : "Marca creada correctamente.",
...
/**
 * Comprueba si existe una marca con el ID especificado
 * @param id El ID de la marca a verificar
 * @return true si existe, false si no
 */
private boolean marcaExiste(String id) {
    ...
    System.out.println("Buscando marca con ID: " + id);

    // Usa el método searchById para buscar en la tabla de marcas
    Map<String, String> marcaData = tc.searchById(params);  // Asumiendo que tc es tu controlador de base de datos para la tabla de marcas

    if (marcaData != null && !marcaData.isEmpty()) {
        System.out.println("Se encontró una marca con el ID: " + id + " -> " + marcaData);
        return true;
    } else {
        System.out.println("No se encontró ninguna marca con el ID: " + id);
        return false;
    }
}
    ...
    public int imBorrar(String crud) {
     if (IdMarcas.getText().trim().isEmpty() || marca.getText().trim().isEmpty()) {
        ...
        int idMarcaActual = obtenerIdMarcasActual();
        if (idMarcaActual <= 0) {
            JOptionPane.showMessageDialog(this, "ID inválido o no seleccionado.", "Error", JOptionPane.ERROR_MESSAGE);
            return -1;
        }

        int confirmacion = JOptionPane.showConfirmDialog(this, "¿Está seguro de que desea eliminar esta marca?", "Confirmar", JOptionPane.YES_NO_OPTION);
        ...
        registro.put("id", String.valueOf(idMarcaActual));
        registrosParaBorrar.add(registro);

        int resultado = tc.deleteReg(registrosParaBorrar);
        if (resultado > 0) {
            JOptionPane.showMessageDialog(this, "Marca eliminada con éxito.", "Éxito", JOptionPane.INFORMATION_MESSAGE);
        } else {
            JOptionPane.showMessageDialog(this, "No se pudo eliminar la marca.", "Error", JOptionPane.ERROR_MESSAGE);
        }
...
Frame parentFrame = (Frame) SwingUtilities.getWindowAncestor(this);
    // Lista de columnas para mostrar en el formulario de búsqueda de marcas
    List<String> columnasParaMarcas = Arrays.asList("id", "marca");
    Form_Buscar buscadorMarcas = new Form_Buscar(parentFrame, true, tc, "MARCAS", columnasParaMarcas);

    buscadorMarcas.setOnItemSeleccionadoListener(this);
    // Muestra el formulario de búsqueda
    buscadorMarcas.setVisible(true);
...
public int imSiguiente() {
    ...
    Map<String, String> registro = tc.navegationReg(String.valueOf(idMarcas), "NEXT");
    return procesarRegistroNavegacion(registro);
}
...
public int imAnterior() {
    ...
    Map<String, String> registro = tc.navegationReg(String.valueOf(idMarcas), "PRIOR");
    return procesarRegistroNavegacion(registro);
}
...
public void onItemSeleccionado(Map<String, String> datosSeleccionados) {
    ...
    String nombreMarca = datosSeleccionados.get("Descripcion"); // 'Descripcion' se usa como clave para el nombre de la marca

    if (idStr != null && !idStr.isEmpty()) {
        try {
            int idMarca = Integer.parseInt(idStr); // Convertir el ID a entero
            // Buscar detalles adicionales de la marca si es necesario
            List<Map<String, String>> registros = tc.buscarPorIdGenerico("MARCAS", "id", idMarca);
            ...
            if (!registros.isEmpty()) {
                ...
                final String descripcionMarca = registro.get("descripcion"); // Recuperar la descripción si está disponible

                // Actualizar la UI en el hilo de Swing
                SwingUtilities.invokeLater(() -> {
                    IdMarcas.setText(idStr);
                    marca.setText(nombreMarca);
                    textdescripcion.setText(descripcionMarca); // Actualizar campo de descripción
                    idMarcas = idMarca; // Guardar el ID de la marca actual
                });
            } else {
                JOptionPane.showMessageDialog(null, "No se encontró una marca con el Código especificado.", "Información", JOptionPane.INFORMATION_MESSAGE);
            }
        } catch (NumberFormatException ex) {
