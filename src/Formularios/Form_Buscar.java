...
public class Form_Buscar extends javax.swing.JDialog {
   ...
   // Listener para comunicarse con el formulario llamador
    ...
    public void setOnItemSeleccionadoListener(InterfaceUsuario listener) {
    this.seleccionadoListener = listener;
    }

    public Form_Buscar(Frame parent, boolean modal, DBTableController controlador, String nombreTabla, List<String> columnasDeBusqueda) {
        super(parent, modal);
...
private void filtrar() {
    ...
    List<Map<String, String>> resultados = controlador.buscarPorColumna(nombreTabla, columnasDeBusqueda, textoBusqueda);

    for (Map<String, String> fila : resultados) {
        Object[] filaDatos = new Object[columnasDeBusqueda.size()];
        for (int i = 0; i < columnasDeBusqueda.size(); i++) {
            filaDatos[i] = fila.get(columnasDeBusqueda.get(i));
        }
        modelo.addRow(filaDatos);
    }

    if (modelo.getRowCount() == 0) {
        // Verificar si la ventana aún está visible antes de mostrar el diálogo
        if (this.isVisible()) {
...
private void seleccionarItem(int fila) {
    ...
    for (int i = 0; i < modelo.getColumnCount(); i++) {
        ...
        datosSeleccionados.put(nombreColumna, valorColumna.toString());
